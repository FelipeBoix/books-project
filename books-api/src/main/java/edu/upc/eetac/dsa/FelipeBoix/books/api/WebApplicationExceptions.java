...
import edu.upc.eetac.dsa.FelipeBoix.books.api.model.BooksError;
	...
	public class WebApplicationExceptionMapper implements
		...
		public Response toResponse(WebApplicationException exception) { //construimos la respuesta
			...
			return Response.status(error.getStatus()).entity(error) //crea la respuesta a manopla con el estado y el mensaje que le hemos puesta en esta excepcion
					.type(MediaType.BOOKS_API_ERROR).build(); //status el codigo http de la respuesta, el entity es el body de la respuesto y el tipo es el content type, y el build permite crear la respuesta
		}
